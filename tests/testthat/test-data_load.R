...
test_that("data_load runs in demo mode when both json_file and domain_file are NULL", {
...
test_that("data_load throws error if only one of json_file or domain_file is NULL", {
  expect_error(data_load(json_file, NULL, NULL))
  expect_error(data_load(NULL, domains_file, NULL))
})

test_that("data_load reads user-specified files correctly", {
  result <- data_load(json_file, domains_file, NULL)
...
test_that("data_load reads user-specified look-up table correctly", {
  result <- data_load(json_file, domains_file, look_up_file)
