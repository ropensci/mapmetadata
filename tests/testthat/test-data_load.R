...
csv_file <- system.file("inputs/360_NCCHD_Metadata.csv",
                        package = "mapmetadata")
look_up_file <- system.file("inputs/look_up.csv",
                            package = "mapmetadata")
domains_file <- system.file("inputs/domain_list_demo.csv",
                            package = "mapmetadata")
...
test_that("data_load throws error if only one of csv_file or domain_file
          is NULL", {
...
test_that("data_load reads user-specified files correctly", {
  ...
  expect_equal(result$domain_list_desc,
               tools::file_path_sans_ext(basename(domains_file)))
})
