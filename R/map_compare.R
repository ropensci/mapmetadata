...
#' )
#' }
map_compare <- function(session_dir, session1_base, session2_base, metadata_file, domain_file, output_dir = session_dir) {
  timestamp_now_fname <- format(Sys.time(), "%Y-%m-%d-%H-%M-%S")

  # DEFINE INPUTS ----

  csv_1a <- read.csv(paste0(session_dir, "/", "MAPPING_LOG_", session1_base, ".csv"))
  csv_2a <- read.csv(paste0(session_dir, "/", "MAPPING_LOG_", session2_base, ".csv"))
  csv_1b <- read.csv(paste0(session_dir, "/", "MAPPING_", session1_base, ".csv"))
  csv_2b <- read.csv(paste0(session_dir, "/", "MAPPING_", session2_base, ".csv"))
  ...
  metadata_file_base_0suffix <- sub("_Structural_Metadata.csv$", "", metadata_file_base)
  ...
  ses_join <- left_join(csv_1b, csv_2b, suffix = c("_ses1", "_ses2"), join_by(data_element))
  ...
  # SAVE TO NEW CSV ----
  output_fname <- paste0(output_dir, "/CONSENSUS_MAPPING_", gsub(" ", "", dataset_name), "_", table_name, "_", timestamp_now_fname, ".csv")
