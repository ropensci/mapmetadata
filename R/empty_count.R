...
#' @param dataframe The dataframe representing the metadata, with 'Section' column as factor
...
empty_count <- function(dataframe) {
  ...
  empty_count_df <- dataframe %>%
    ...
    pivot_longer(cols = c(no_count, empty_count), names_to = "Empty", values_to = "N_Variables") %>%
